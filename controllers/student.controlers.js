...
export async function register(req, res) {
    ...
    try {
        ...
        if(isEmailExist) return sendResponse(res, 400, false, null, 'Student with this email already exist')
        
        const newId = await generateUniqueCode(9)
        const userId = `EDU${newId}ST`
        ...
        //refresh token
        const refreshTokenExist = await RefreshTokenModel.findOne({ accountId: newStudent.userId })
        if(refreshTokenExist){
            refreshTokenExist.refreshToken = refreshToken
            await refreshTokenExist.save()
        } else {
            await RefreshTokenModel.create({
                accountId: newStudent?.userId,
                refreshToken,
        ...
        res.cookie('eduafricaauthid', newStudent?.userId, {
            httpOnly: true,
            sameSite: 'None',
            secure: true,
        ...
        //send user data
        const { password: userPassword, verified, isBlocked, accountSuspended, noOfLoginAttempts, temporaryAccountBlockTime, resetPasswordToken, completedOnboarding, _id, ...userData } = newStudent._doc;
        ...
        console.log('UNABLE TO CREATE NEW STUDENT', error)
        sendResponse(res, 500, false, null, 'Unable to create new student account')
    }
}
...
export async function resetPassword(req, res) {
    ...
    try {
        const getUser = await StudentModel.findOne({
            resetPasswordToken,
    ...
    try {
        if (accessToken) {
            try {
                ...
                const user = await StudentModel.findOne({ userId: decoded.id });
...
async function handleTokenRefresh(res, accountId) {
    ...
    const user = await StudentModel.findOne({ userId: accountId });
    if (!user) return sendResponse(res, 404, false, null, 'User not found');
